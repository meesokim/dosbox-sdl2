	...
	private class LocalTouch {
        ...
        public void onDown(int id, float x, float y){
			...
			if (x < 200 && y < 200)
				...
				if (y < h/2)
					SDLActivity.onNativeKeyDown(key = KeyEvent.KEYCODE_ENTER);
				else if (x > w-200) 
        ...
        public void onMove(float x, float y){
			int len = (int)Math.sqrt((x0-x)*(x0-x)+(y0-y)*(y0-y));
			int degree = (int) (Math.atan2(x0 - x, y0 - y) * 180 / Math.PI);
			if (len > 30 && cursor) { 
				Log.v("SDL", "dx=" + (int) (x0 - x) + ",dy=" + (int)(y0 - y) + ",degree:" + (int) degree + ",len=" + len);
				...
				if (degree >= 180-50 || degree < -180+50) {
